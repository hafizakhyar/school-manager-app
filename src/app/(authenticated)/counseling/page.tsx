...
import { HeartHandshake, Search, Save, Upload, CheckSquare, Square } from "lucide-react";
...
export default function CounselingPage() {
  ...
  const [counselingRecords, setCounselingRecords] = useState<{ [studentId: string]: any }>({});
  ...
  useEffect(() => {
    ...
    const fetchStudentsAndCounseling = async () => {
      setLoading(true);
      try {
        ...
        studentSnap.forEach(d => {
          ...
          if (isMatch) {
            studentList.push({ 
              ...
              classId: sClassId,
              counselingData: data.counselingData || {}
            });

            const allCounseling = data.counselingData || {};
            const currentRecord = allCounseling[docKey];
        ...
        setCounselingRecords(recordsMap);
        setSelectAll(false);
      } catch (err) {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      for (const student of students) {
        ...
        const dataToSave = counselingRecords[student.id] || {};
        const existingCounseling = student.counselingData || {};
      ...
      alert("Data Bimbingan Konseling berhasil disimpan!");
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 pb-16">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Bimbingan Konseling (BK)</h1>
          <p className="text-sm text-slate-400 mt-1">Catatan bimbingan, konseling, dan penanganan kasus siswa.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{saving ? "Menyimpan..." : "Simpan Catatan BK"}</span>
          </button>
        </div>
      </div>
                      ...
                      <tr key={student.id} className="hover:bg-slate-900/30 transition-colors">
                        ...
                        <td className="py-4 px-6">
                          <select 
                            ...
                            onChange={e => handleChangeField(student.id, "category", e.target.value)}
                            className="w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-amber-400 font-semibold outline-none focus:border-indigo-500"
                          >
                            <option value="Disiplin">Disiplin</option>
                            <option value="Akademik">Akademik</option>
                            <option value="Pribadi">Pribadi / Sosial</option>
                        ...
                        <td className="py-4 px-6">
                          <input 
                            ...
                            value={record.issue}
                            onChange={e => handleChangeField(student.id, "issue", e.target.value)}
                        ...
                        <td className="py-4 px-6">
                          <input 
                            type="text"
                            placeholder="Uraikan tindakan / solusi..."
                            value={record.action}
                            onChange={e => handleChangeField(student.id, "action", e.target.value)}
                            className="w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500"
                          />
                        </td>
                      </tr>
